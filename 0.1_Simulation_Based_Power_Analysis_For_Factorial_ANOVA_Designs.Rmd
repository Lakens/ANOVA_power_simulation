...
Because the effect size in the first study could be considered 'medium' based on the benchmarks by @cohen_statistical_1988, does it make sense to plan for a 'medium' effect size in either the between or within ANOVA design?
...
Popular power analysis software such as G\*Power [@faul_gpower_2007] also allows researchers to enter the effect size for the power analsysi as partial eta-squared ($\eta_p^2$).
...
```{r mean-plot2, fig.width=7, fig.height=4, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.cap="Vizualization for the expected means and confidence intervals for the two conditions."}
...
Since there are only two groups in this example, the results for the statistical power of the independent comparison is identical, but the effect size is `r power_result$pc_results$effect_size`, which is the effect size in Cohen's d. 
...
\end{equation}
Here *a* is the number of within-participant levels, $\rho$ is the correlation between the measurements, and the formula assumes normal distributions and compound symmetry, and ignores the difference in degrees of freedom between the two types of tests, so it is a rough (but useful) approximation.
...
Simulations show we have would have exactly the same power for a cross-over (also called 'disordinal') interaction if we halved the sample size per group from 80 to 40.
...
There is no equivalent Cohen's $f_z$ for a within-participant ANOVA, as there is a Cohen's $d_z$ for a dependent *t*-test. Instead, the value for lambda ($\lambda$) is adjusted based on the correlation. For a one-way within-participant design lambda is calculated as in Equation \@ref(eq:lambda), multiplied by *u*, a correction for within-designs, calculated as:
\begin{equation}
u = \frac{k}{1-\rho}
\end{equation}
...
Violations of the homogeneity assumption impact Type 1 error rates [@delacre_why_2018].    
...
Although the power for the cross-over interaction is high (`r power_result$main_results$power[3]`%), the power for the independent between-participant comparisons where there is a difference is only `r power_result$pc_results$power[2]`%, while the power for correlated between-participant effects is `r power_result$pc_results$power[1]`%. 
