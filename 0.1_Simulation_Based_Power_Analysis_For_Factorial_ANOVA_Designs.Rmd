...
Assuming means of 1, 0.5, and 0, with a standard deviation of 2, and 80 participants per condition, power for the between design was `r power_result_1$main_results$power`%.
...
For a one-way within-participant design lambda is calculated as in Equation \@ref(eq:lambda), multiplied by *u*, a correction for within-designs, calculated as:
\begin{equation}
u = \frac{k}{1-\rho}
\end{equation}
...
Simulations can be used to see that because of the positive correlation between dependent variables, the effect size $\eta_p^2$ is much larger for the within design (`r power_result_within_1$main_results$effect_size`) than for the between design (`r power_result_1$main_results$effect_size`). 
...
power_result_holm_correction <- ANOVA_power(design_result_holm_correction, alpha_level = 0.05, p_adjust = "holm", nsims = nsims, seed = 2019, verbose = FALSE)
```
...
Statistical power is the ong run probability of observing a *p*-value smaller than the alpha level.
